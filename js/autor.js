...
// + Sticky hero al hacer scroll (foto pequeña + stats en tira)
// + Biografía visible en modo expandido
// ============================================================
...
function _buildOverlay() {
  ...
  // ── Sticky mini-hero (visible al hacer scroll) ────────────
  const stickyBar = el('div', { className: 'autor-sticky-bar', attrs: { id: 'autor-sticky-bar', 'aria-hidden': 'true' } });
  overlay.appendChild(stickyBar);

  // ── Cuerpo scrollable ────────────────────────────────────
  const body = el('div', { className: 'autor-body', attrs: { id: 'autor-body' } });
...
// ── Scroll → sticky bar ───────────────────────────────────────
function _initStickyBehavior(body, hero, stickyBar) {
  const THRESHOLD = 80; // px desde el top del hero para activar sticky

  body.addEventListener('scroll', () => {
    const heroBottom = hero.getBoundingClientRect().bottom;
    const overlayTop = body.getBoundingClientRect().top;
    const collapsed  = (heroBottom - overlayTop) < THRESHOLD;

    stickyBar.classList.toggle('autor-sticky-bar--visible', collapsed);
    hero.classList.toggle('autor-hero--collapsed', collapsed);
  }, { passive: true });
}
...
async function _loadAutorData(userId, overlay) {
  ...
  try {
    ...
    // ── Hero expandido (estado normal) ────────────────────────
    const anonNum  = profile.anonymous_number ?? '?';
    const anonName = `Anónimo_${anonNum}`;

    const hero = el('div', { className: 'autor-hero', attrs: { id: 'autor-hero' } });

    // Avatar grande
    const avatarWrap = el('div', { className: 'autor-avatar-wrap' });
    const avatar     = el('div', { className: 'autor-avatar autor-avatar--lg' });
    ...
    // Nombre
    hero.appendChild(el('h2', { className: 'autor-name', textContent: anonName }));
    ...
    // Estadísticas
    const stats = el('div', { className: 'autor-stats', attrs: { id: 'autor-stats' } });
    ...
    if (_currentUser && !isSelf) {
      ...
      followBtn.addEventListener('click', () => showToast('Inicia sesión para seguir usuarios.', 'info'));
      hero.appendChild(followBtn);
    }

    // ── Biografía (solo en modo expandido) ────────────────────
    if (profile.bio) {
    ...
    // ── Poblar sticky bar ──────────────────────────────────────
    const stickyBar = document.getElementById('autor-sticky-bar');
    if (stickyBar) {
      _renderStickyBar(stickyBar, profile, anonName, { followers: followers ?? 0, likes: totalLikes, pubs: totalPubs ?? 0 });
    }

    // Activar comportamiento sticky
    _initStickyBehavior(body, hero, stickyBar);
...
// ── Sticky bar: foto pequeña + nombre + stats en tira ─────────
function _renderStickyBar(bar, profile, anonName, stats) {
  while (bar.firstChild) bar.removeChild(bar.firstChild);

  // Avatar mini
  const avatar = el('div', { className: 'autor-avatar autor-avatar--sm' });
  if (profile.avatar_url) {
    const img = document.createElement('img');
    img.src = profile.avatar_url; img.alt = 'Avatar'; img.loading = 'lazy';
    avatar.appendChild(img);
    ...
    avatar.appendChild(el('span', {
      className: 'autor-avatar__initials autor-avatar__initials--sm',
      textContent: getInitials(profile.full_name),
    }));
  }
  bar.appendChild(avatar);

  // Info: nombre + stats inline
  const info = el('div', { className: 'autor-sticky-info' });
  info.appendChild(el('span', { className: 'autor-sticky-name', textContent: anonName }));

  const statsRow = el('div', { className: 'autor-sticky-stats' });
  const mkMini = (val, label) => {
    const s = el('span', { className: 'autor-sticky-stat' });
    s.innerHTML = `<strong>${val}</strong> ${label}`;
    return s;
  };
  statsRow.appendChild(mkMini(stats.followers, 'seg.'));
  statsRow.appendChild(mkMini(stats.likes,     'likes'));
  statsRow.appendChild(mkMini(stats.pubs,      'pub.'));
...
function _renderDropdown(userId, profile) {
  ...
  if (!isSelf) {
    dd.appendChild(_mkDropdownItem('Reportar perfil', 'var(--danger, #ef4444)', () => {
      dd.hidden = true; _reportProfile(userId);
    }));
  }
  ...
  if (isAdmin && !isSelf) {
    ...
    if (!isSuspended) {
      [1, 3, 7, 30].forEach(days => {
        dd.appendChild(_mkDropdownItem(`Suspender ${days} día${days > 1 ? 's' : ''}`, 'var(--warning, #f59e0b)', () => {
          dd.hidden = true; _suspendUser(userId, days);
        }));
      });
    } else {
      dd.appendChild(_mkDropdownItem('Desuspender', 'var(--success, #22c55e)', () => {
        dd.hidden = true; _unsuspendUser(userId);
      }));
    }
  }
...
async function _loadAutorFeed(userId, container) {
  ...
  data.forEach(c => {
    feedBuildCard(c, container, false, false,
      likeMap[c.id]    || 0,
      commentMap[c.id] || 0,
...
async function _toggleFollow(targetId, btn, statsEl) {
  ...
  try {
    ...
    const allValues = statsEl?.querySelectorAll('.autor-stat__value');
    if (allValues?.[0]) {
      const n = parseInt(allValues[0].textContent) || 0;
      allValues[0].textContent = String(isFollowing ? Math.max(0, n - 1) : n + 1);
    }

    // Actualizar en sticky bar
    const stickyStats = document.querySelectorAll('.autor-sticky-stat strong');
    if (stickyStats?.[0]) {
      const n = parseInt(stickyStats[0].textContent) || 0;
      stickyStats[0].textContent = String(isFollowing ? Math.max(0, n - 1) : n + 1);
    }
  } catch {
...
async function _reportProfile(userId) {
  ...
  try {
    await sb.from('reports').insert({
      reporter_id: _currentUser.id, reported_type: 'profile',
      reported_id: userId, reason: reason.trim(),
    });
...
export function bindAvatarToAutor(avatarEl, userId, fromView = 'feed') {
  ...
  avatarEl.addEventListener('click', (e) => { e.stopPropagation(); openAutor(userId, fromView); });
