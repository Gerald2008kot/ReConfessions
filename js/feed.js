// js/feed.js  (actualizado)
// Agregado: paginación infinita, ordenar (recientes/comentados/votados), confesiones destacadas (pin)
// ============================================================
...
let activeSort          = 'recent'; // 'recent' | 'comments' | 'likes'
...
export async function loadConfessions(containerEl, userId = null) {
  ...
  // Reset paginación al recargar desde cero
  ...
  // Confesiones destacadas (pinned) primero — solo en feed principal
  ...
  // Re-iniciar infinite scroll tras reset
  if (!containerEl && !userId) {
...
// ── Fetch una página de confesiones ──────────────────────────
async function fetchConfessionsPage(page = 0, userId = null) {
  ...
  if (userId) {
    ...
    // Ordenar — like_count/comment_count sólo si existen como columnas; fallback a created_at
    switch (activeSort) {
      case 'likes':
        query = query.order('created_at', { ascending: false }); // fallback hasta tener columna
        break;
      case 'comments':
        query = query.order('created_at', { ascending: false }); // fallback hasta tener columna
        break;
  ...
  if (error) { showToast('Error cargando confesiones.', 'error'); return { data: [] }; }
